...
export async function createFighterTables(): Promise<void> {
  const query = `
...
export async function findFighterById(fighterId: number): Promise<Fighter | null> {
  const query = `SELECT * FROM ufc_fighter WHERE fighter_id = $1`;
...
export async function getFighterImage(fighterId: number): Promise<string | null> {
  const query = `
  ...
  try {
    const result = await pool.query<{ img_url: string }>(query, [fighterId]);
    if (result.rows.length > 0) {
